...
for i in range(k): # optimization loop
    ...
    ## Progress ##
    np.savetxt('ProgDir/in_iter_%d.csv' % (i + 1), x)
    np.savetxt('ProgDir/out_iter_%d.csv' % (i + 1), -y_step)
    np.savetxt('ProgDir/mu_iter_%d.csv' % (i + 1), gpr_step.predict(des_grid)[0].reshape(np.shape(des_grid_xx)), delimiter=",")
    np.savetxt('ProgDir/sigma_iter_%d.csv' % (i + 1), gpr_step.predict(des_grid)[1].reshape(np.shape(des_grid_xx)), delimiter=",")
